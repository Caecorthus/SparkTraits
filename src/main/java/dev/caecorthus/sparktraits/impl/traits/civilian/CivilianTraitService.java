 ...
 * SparkTraits 平民阵营天赋的共享规则入口；保持原始阵营判断，避免扩大到内鬼或中立。
 */
public final class CivilianTraitService {
    ...
    public static void register() {
        ServerTickEvents.END_WORLD_TICK.register(CivilianTraitService::tickWorld);
    }

    public static boolean canSelectCivilianTrait(Role role, Collection<Identifier> selectedTraits) {
        return EffectiveTraitService.isOriginalCivilian(role)
                && !EffectiveTraitService.hasImpostor(safeTraits(selectedTraits));
    }

    /** Civilian-trait gate for effects that should not attach to Undercover.
     *  不给卧底使用的平民阵营词条资格入口。 */
    public static boolean canSelectNonUndercoverCivilianTrait(Role role, Collection<Identifier> selectedTraits) {
        return canSelectCivilianTrait(role, selectedTraits) && !EffectiveTraitService.isUndercover(role);
    }
    ...
    public static boolean canSelectMoneyTree(
        ...
        return canSelectCivilianTrait(role, selectedTraits)
    ...
    public static boolean shouldPreventSocialMoodDrain(Collection<Identifier> traits, int nearbyOtherPlayers) {
        ...
        return (safeTraits.contains(CivilianTraits.EXTROVERTED) && nearbyOtherPlayers >= 2)
                || (safeTraits.contains(CivilianTraits.INTROVERTED) && nearbyOtherPlayers <= 1);
    }
    ...
    public static float socialMoodAdjustedMood(float currentMood, float proposedMood, PlayerEntity player) {
        ...
        if (!canSelectCivilianTrait(game.getRole(player), traits)) {
            return proposedMood;
        }
    ...
    public static boolean shouldPreventGunMoodPenalty(Role role, Collection<Identifier> traits) {
        Collection<Identifier> safeTraits = safeTraits(traits);
        return safeTraits.contains(CivilianTraits.FOCUS) && canSelectFocus(role, safeTraits);
    }
    ...
    static boolean canReceiveMoneyTree(ServerPlayerEntity player, GameWorldComponent game) {
        ...
        if (!traits.hasActiveTrait(CivilianTraits.MONEY_TREE)) {
            return false;
        }
